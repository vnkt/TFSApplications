...
namespace CodeCoverageCheck
{
    ...
    public sealed class CodeCoverageCheckActivity : CodeActivity<int> //returns integer
    {
        ...
        protected override int Execute(CodeActivityContext context)
        {
            ...
            {
                ITestRunCoverage[] source = coverageAnalysisManager.QueryTestRunCoverage(currentRun.Id, CoverageQueryFlags.Modules); //Getting code coverage
                coveredBlocks += source.Sum((ITestRunCoverage c) => c.Modules.Sum((IModuleCoverage m) => m.Statistics.BlocksCovered));  
                skippedBlocks += source.Sum((ITestRunCoverage c) => c.Modules.Sum((IModuleCoverage m) => m.Statistics.BlocksNotCovered));
            }
